...
<body>
    ...
    <div class="container">
        ...
        <script type="text/javascript">
            var userid = {{ $user->id }};
            var loggedInTeacherId = {{ session('LoggedTeacherInfo') }};

            // Function to fetch messages
            function fetchMessages() {
                $.ajax({
                    type: 'POST',
                    url: '/teacher/fetch-user-messages',
                    data: {
                        receiver_id: userid,
                        _token: '{{ csrf_token() }}'
                    },
                    success: function(data) {
                        ...
                        $.each(messages, function(index, message) {
                            ...
                            messageHtml += '<span class="message-data-time">' + message.created_at +
                                '</span>';
                            messageHtml += '</div>';
                            messageHtml += '<div class="message ' + (message.sender_id ==
                                    loggedInTeacherId ? 'my-message' : 'other-message') + '">' + message
                                .message + '</div>';
                            messageHtml += '</li>';
                            $('.chat-history ul').append(messageHtml);
                        });

                        // Scroll to the bottom of the chat history after messages are loaded
            ...
            $(document).ready(function() {
                ...
                $('#sendMessageButton').on('click', function() {
                    ...
                    $.ajax({
                        type: 'POST',
                        url: '/teacher/send-message-to-user',
                        data: {
                            message: message,
                            receiver_id: userid, // Assuming the teacher's ID is the receiver ID
